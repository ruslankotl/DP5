...
class ConfSearchMethod(BaseConfSearch):
    ...
    def __init__(self, inputs, settings):
        ...
        if not self.settings['executable']['tinker']:
            logger.warning("No Tinker executable specified. Please check your config file.")
    ...
    def prepare_input(self):
        for input in self.inputs:
            ...
            outp = subprocess.check_output(convinp + input + '.sdf',
                                           shell=True)
        return self.inputs
    ...
    def _run(self):
        ...
        for input in self.inputs:
            if (os.path.exists(f"{input}.tout") 
                and os.path.exists(f"{input}.arc")):
                ...
                with open(f'{input}.tout', 'w') as tout:
                    subprocess.run(f"{self.tinker_exe} {input}.xyz {self.ff_params} 0 10 20 0.00001",
                                    text=True, shell=True, check=True, stdout=tout)
    ...
    def _read_arc(self, file):
        ...
        def GetAtomSymbol(AtomNum):
            ...
            if AtomNum > 0 and AtomNum < len(Lookup):
                ...
                logger.error(f"Element with atomic number {AtomNum} not supported, will return 0")
                return 0
