...
def conf_search(mols, config) -> List:
    ...
    try:
        method = Method.ConfSearchMethod(final_names,config)
        logger.debug(f"Initialised {config['method']}")
    except AttributeError:
        logger.critical(f"{config['method']} does not implement ConfSearchMethod. Terminating.")
        sys.exit(1)
    ...
    for mol, data in zip(input_names, cs_data):
        ...
        if f'{mol}rot' in flipped_inputs:
            ...
            logger.debug(f"{len(rot_data.conformers)} rotated conformers read for {mol}rot")
        ...
        all_conformers = [(energy, coordinates) for energy, coordinates in all_conformers 
                            if energy < emin + config['energy_cutoff']]
        logger.debug(f"{len(all_conformers)} total conformers read within {config['energy_cutoff']:.1f} kJ/mol for {mol}")
        

        if len(all_conformers) > config['conf_per_structure'] and config['conf_prune']:
            logger.info(f'Conformer pruning required for {mol}')
            unique_conf_ids, cutoff = conf_prune(conformers=coords, 
                                                 cutoff=config['rmsd_cutoff'],
                                                 conf_limit=config['conf_per_structure'])
            
            all_conformers = [conformer for conformer, unique 
                              in zip(all_conformers, unique_conf_ids)
                              if unique]
