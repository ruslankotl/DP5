...
def dft_calculations(mols, workflow, config):
    ...
    try:
        ...
        logger.critical(
            f"No DFT module {config['method']} found. Terminating.")
        sys.exit(1)
    ...
    if workflow['dft_nmr']:
        ...
        atoms, geometries, energies, shieldings, shielding_labels = method.nmr(
            mols)
        for mol, m_atoms, m_geoms, m_energies, m_shieldings, m_labels in zip(
            ...
            mol.C_pred, mol.C_labels, mol.H_pred, mol.H_labels = \
                _shielding_to_shift(m_atoms, m_shieldings,
                                    config['c13_tms'], config['h1_tms'])
...
def _shielding_to_shift(atoms, shieldings, c13_ref, h1_ref):
    ...
    for i, (atom, shielding) in enumerate(zip(atoms, shieldings_by_atom), start=1):
        if atom == 'C':
            shift = [(c13_ref-c_sh) / (1-(c13_ref*(10**-6)))
                     for c_sh in shielding]
            C_pred.append(shift)
            C_labels.append("C%i" % i)
        if atom == 'H':
            shift = [(h1_ref-h_sh) / (1-(h1_ref*(10**-6)))
                     for h_sh in shielding]
            H_pred.append(shift)
