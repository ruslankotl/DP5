...
LOGLEVEL_CHOICES = tuple(level.lower()
                         for level in logging._nameToLevel.keys())

DEFAULT_BASE_CONFIG_PATH = (
    Path(__file__).parent.parent / 'config/default_config.toml').resolve()
...
def main():
    ...
    parser.add_argument('-s', '--structure_files', nargs='+', default=[], type=str, help="One or more SDF file for the structures to be verified by DP4. At least one\
    is required, if automatic diastereomer generation is used.")

    parser.add_argument('-n', "--nmr_file", nargs='+', help="Experimental NMR description, assigned\
    with the atom numbers from the structure file")

    parser.add_argument("-c", "--config", help="Load a config file",
                        type=str, default=DEFAULT_BASE_CONFIG_PATH)

    parser.add_argument(
        '-o', '--output', help="Output directory for calculations, default is current working directory.", default='')

    parser.add_argument('-i', '--input_type', help='Input file format. Default is sdf.',
                        choices=['sdf', 'smiles', 'smarts', 'inchi'], default='sdf')
    ...
    parser.add_argument('--stereocentres', nargs='*', default=[], type=int,
    ...
    logger = setup_logger(
        name=__package__, level=config['log_level'].upper(), filename=args.log_filename, propagate=True)
    ...
    logger.info(
        f"Diastereomer generation required: {config['workflow']['generate']}")
    logger.info(
        f"Conformational search required: {config['workflow']['conf_search']}")
    logger.info(
        f"DFT geometry optimisation required: {config['workflow']['dft_opt']}")
    logger.info(
        f"DFT energy calculation required: {config['workflow']['dft_energies']}")
    logger.info(
        f"DFT shielding calculation required: {config['workflow']['dft_nmr']}")
