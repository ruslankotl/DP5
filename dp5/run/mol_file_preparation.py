...
def read_textfile(input_file: str, input_type: str):
    ...
    input_readers = {'smiles': Chem.MolFromSmiles,
...
def _generate_diastereomers(mol:Chem.rdchem.Mol, mutable_atoms: list[int] | None = None,double_bonds: bool = False) -> list[Chem.rdchem.Mol]:
    """
...
def prepare_inputs(input_files: list[str], input_type: str, stereocentres: list[int], workflow:dict) -> list[str]:
    """
    ...
    if input_type == 'sdf':
        ...
        input_files = [f'{input_type}_mol_{i:03}_.sdf' for i, mol in enumerate(range(len(mols)), start=1)]
