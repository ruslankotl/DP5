...
from dp5.nmr_processing.NMR import NMRData


def assign_proton(mol: Molecule, nmr_data: NMRData):
    """
    take undercooked NMR data, assign given the structure of existing compound
    args for mol: Hshifts, Hlabels
    args for NMRData: Hshifts
    NMRData integrals have to be recalculated for every candidate
    
    """
    H_shifts = []
    H_labels = []
    
    finish_processing(mol.rdkit_mols[0], nmr_data)

    assigned_shifts, assigned_peaks, assigned_labels,scaled_shifts \
        = iterative_assignment(NMRData.Hshifts,mol.H_pred,mol.H_labels,NMRData.protondata["integrals"], mol.rdkit_mols[0])
    return H_exp

def iterative_assignment(exp_peaks,calculated_shifts, H_labels,rounded_integrals, rdkit_mol):
    ...
    while old_assigned_shifts != new_assigned_shifts:
        ...
        m_protons = methyl_protons(rdkit_mol)
