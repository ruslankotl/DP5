...
def BIC_minimisation_region_full(
    ...
    while len(copy_peaks) > 0:
        ...
        params.add(
            "A" + str(maxpeak),
            value=total_spectral_ydata[maxpeak],
            min=0,
            max=1,
            vary=True,
        )
        ...
        params.add(
            ...
            value=maxpeak,
            vary=True,
            min=maxpeak - 4 * corr_distance,
            max=maxpeak + 4 * corr_distance,
        )
    ...
    if maxintegral > inty:
        ...
        while (intmodel / inty < 0.99) or (intmodel / inty > 1.01):
            ...
            for f in fitted_peaks:
                params["std" + str(f)].set(
                    value=params["std" + str(f)] * inty / intmodel
                )
            ...
            for f in fitted_peaks:
                params["A" + str(f)].set(
                    value=params["A" + str(f)]
                    * region_y[int(params["mu" + str(f)]) - region[0]]
    ...
    for peak in fitted_peaks:
        params["A" + str(peak)].set(
            vary=False,
            min=max(0, params["A" + str(peak)] - 0.01),
            max=min(params["A" + str(peak)] + 0.01, 1),
        )
        params["mu" + str(peak)].set(vary=False)
        params["std" + str(peak)].set(
            vary=False,
            min=min(std_lower, params["std" + str(peak)] - av_std),
...
def multiproc_BIC_minimisation(
    ...
    for ind1 in sorted_regions:
        new_grouped_peaks[ind1], new_grouped_params[ind1], new_sim_y[ind1] = res[
            ind1
        ].get()
        ...
        for ind2, peak in enumerate(group):
            ...
            if ind2 > 0:
                ...
                if peak > group[ind2 - 1] + dist_hz:
                    ...
                    total_params.add(
                        "vregion" + str(newgroupind),
                        value=new_grouped_params[oldgroupind][
                            "vregion" + str(oldgroupind)
                        ],
                    )
            ...
            total_params.add(
                "A" + str(peak), value=new_grouped_params[oldgroupind]["A" + str(peak)]
            )
            total_params.add(
                "std" + str(peak),
                value=new_grouped_params[oldgroupind]["std" + str(peak)],
            )
            total_params.add(
                "mu" + str(peak),
                value=new_grouped_params[oldgroupind]["mu" + str(peak)],
                vary=False,
            )
    ...
    final_peak_regions = np.empty(len(final_grouped_peaks), dtype=object)
    ...
    final_sim_y = np.empty(len(final_grouped_peaks), dtype=object)
