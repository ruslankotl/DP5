...
class DFTMethod(BaseDFTMethod):
    def __init__(self, settings):
        ...
        if shutil.which(self.executable) is None:
            logger.warning(f"Gaussian executable not found at {self.executable}. Calculations will not run!")
    ...
    def nmr_options(self):
        ...
        ultrafine =  ' int=ultrafine' if (self.settings['n_functional']).lower() == 'm062x' else ' '
        solvent = f" scrf=(solvent={self.settings['solvent']})" if self.settings['solvent'] else ' '

        route = (f"# {self.settings['n_functional']}/{self.settings['n_basis_set']}"
    ...
    def e_options(self):
        ...
        ultrafine =  ' int=ultrafine' if (self.settings['e_functional']).lower() == 'm062x' else ' '
        solvent = f" scrf=(solvent={self.settings['solvent']})" if self.settings['solvent'] else ' '


        route = (f"# {self.settings['e_functional']}/{self.settings['e_basis_set']}"
                 f"{ultrafine}{solvent}\n")
    ...
    def opt_options(self):
        ...
        ultrafine =  ' int=ultrafine' if (self.settings['o_functional']).lower() == 'm062x' else ' '
        solvent = f" scrf=(solvent={self.settings['solvent']})" if self.settings['solvent'] else ' '
        calc_fc = ',CalcFC' if self.settings['calc_force_constants'] else ''
        step_size = f",MaxStep={self.settings['opt_step_size']}" if self.settings['opt_step_size']!=30 else ''

        route = (f"# {self.settings['o_functional']}/{self.settings['o_basis_set']}"
        ...
        with open(file, 'r') as f:
            while not completed:
                try:
                    line = next(f)
                    # reads geometry    
                    if line.strip() == 'Standard orientation:' or line.strip() == 'Input orientation:':
                        atoms = []
                        coordinates = []
                        # skips lines
                        for _ in range(5):
                    ...
                    if (line ==  'Error termination request processed by link 9999.' 
                        or 'Normal termination' in line):
                        completed = True

                except StopIteration:
                    logger.error(f"Calculations in {file} have not terminated normally.")
                    break
