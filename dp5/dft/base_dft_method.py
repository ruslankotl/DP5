...
class BaseDFTMethod(ABC):
        ...
        for mol in mols:
            ...
            for i, conf in enumerate(mol.conformers, start=1):
                ...
                if output_name.exists():
                    atoms, coords, *_, completed, converged= self.read_file(output_name)
                    if completed:
                        if calc_type == 'opt':
                            if converged or self.force_converge:
                                ...
                                if coords:
                                    logger.info(f'Reusing partially optimised coordinates for {filename}')
                                    geom = coords
                                output_name.unlink()
                        else:
                            output_files[mol.base_name].append(filename)
    ...
    def _get_prerun_files(self, mols, calc_type: str):
        ...
        if data.is_dir():
            for mol in mols:
                ...
                outputs = [file.with_suffix(self.output_format) for file in inputs
    ...
    def opt(self, mols):
        ...
        for mol_data in files:
            ...
            for file in mol_data:
                mol_atoms, coords, energy, *_, opt_converged = self.read_file(file.with_suffix(self.output_format))

                if opt_converged or self.force_converge:
                    mol_conformers.append(coords)
                    mol_energies.append(energy)
                else:
                    logger.critical(f"Optimisation for {file} has not converged! Terminating...")
                    sys.exit(1)
    ...
    def energy(self, mols):
        ...
        for mol_data in files:
            ...
            mol_energies = []
            for file in mol_data:
                mol_atoms, coords, energy, *_ = self.read_file(file.with_suffix(self.output_format))
                mol_conformers.append(coords)
                mol_energies.append(energy)
    ...
    def nmr(self, mols):
        ...
        for mol_data in files:
            ...
            for file in mol_data:
                mol_atoms, coords, energy, shielding, shielding_label, *flags = self.read_file(file.with_suffix(self.output_format))
                mol_conformers.append(coords)
                mol_energies.append(energy)
    ...
    def _run_calcs(self, jobs: list):
        ...
        if shutil.which(self.executable) is None:
            logger.critical(f"{self} executable not found at {self.executable}. Terminating...")
            sys.exit(1)
    ...
    def read_file(self, file) -> tuple[list[str],list[list[list[float]]], list[float], list[float], bool, bool] :
        ...
        return atoms, coordinates, energy, shieldings, shielding_labels, completed, converged
