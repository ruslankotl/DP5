...
class DP4:
    def __init__(self, output_folder, dp4_config):
        ...
        if stats_model in ('g', 'm'):
            logger.info("Statistical model parameter file: %s" %
                        param_file)
            if param_file in ('none', ''):
                ...
                meanC, stdevC, meanH, stdevH = self.read_parameters(
                    param_file)
        else:
            logger.error("Statistical model not recognised: %s" %
                         stats_model)
            logger.info('Using default statistical model')
        self.H_probability = ProbabilityCalculator(
            mean=meanH, stdev=stdevH)
        self.C_probability = ProbabilityCalculator(
            mean=meanC, stdev=stdevC)
    ...
    def __call__(self, mols):
        ...
        for mol in mols:
            ...
            *H_metadata, H_dp4 = self.dp4_proton(
                mol.predicted_h_shifts(), mol.H_exp, mol.H_labels)
            H_dict = {k: v for k, v in zip(H_keys, H_metadata)}
            mol_dict.update(H_dict)
            H_data.append(H_dp4)

            *C_metadata, C_dp4 = self.dp4_carbon(
                mol.predicted_c_shifts(), mol.C_exp, mol.C_labels)
    ...
    def _dp4(self, calculated: list, experimental: list, labels: list, probability):
        ...
        for calc, exp, lab in zip(calculated, experimental, labels):
            if exp is not None:
                ...
                new_exps.append(exp)
                new_labs.append(lab)
        errors = [scaled-exp for scaled, exp in zip(new_calcs, new_exps)]
        probs = probability(errors)
        # take the product of probabilities
        dp4_score = reduce(lambda x, y: x*y, probs, 1)
...
class ProbabilityCalculator:
    def __init__(self, mean, stdev):
        if isinstance(mean, list) and isinstance(stdev, list):
            ...
            if len(mean) > 1:
                self.probability = lambda error: multiple_gaussian(
                    error, mean, stdev)
            else:
            ...
            raise TypeError(
                "Types of mean and standard deviation do not match!")

    def __call__(self, error):
        if isinstance(error, list):
            dp4_score = [self.probability(e) for e in error]
        else:
            dp4_score = self.probability(error)
        return dp4_score
